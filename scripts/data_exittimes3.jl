...
Np = Int(ceil(8*125000/length(procs)))
dt = 1/(4 * 16 * 10^2)
N = 2^8
α = 0.2
ξ = 2/3
...
a2 = @spawnat procs[1] doexittime3(Np, dt, N, α, ξ, tmax, beta, planning_effort)
a3 = @spawnat procs[2] doexittime3(Np, dt, N, α, ξ, tmax, beta, planning_effort)
a4 = @spawnat procs[3] doexittime3(Np, dt, N, α, ξ, tmax, beta, planning_effort)
a5 = @spawnat procs[4] doexittime3(Np, dt, N, α, ξ, tmax, beta, planning_effort)
a6 = @spawnat procs[5] doexittime3(Np, dt, N, α, ξ, tmax, beta, planning_effort)
a7 = @spawnat procs[6] doexittime3(Np, dt, N, α, ξ, tmax, beta, planning_effort)
a8 = @spawnat procs[7] doexittime3(Np, dt, N, α, ξ, tmax, beta, planning_effort)
a9 = @spawnat procs[8] doexittime3(Np, dt, N, α, ξ, tmax, beta, planning_effort)

path = datadir("sims","zeromodes", savename("ET3l2normR12R13", params, "jld2"));
@alert begin
