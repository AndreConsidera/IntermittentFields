...
for i in 1:nproc
    a[i] = @spawnat procs[i]  doexittime3(Np, dt, N, α, ξ, tmax, beta, planning_effort, kernel)
end

path = datadir("sims", "zeromodes", string(kernel), savename("ET3l2normR12R13", params, "jld2"));
@alert begin    
    for i in 1:nproc
